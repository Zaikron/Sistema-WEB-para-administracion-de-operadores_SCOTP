<?php
      ...
      function buscarF($busVervar) {
            $con = mysqli_connect('localhost','Anthony', 'supersayan10000', 'scotp');
       
            $sql = mysqli_query($con, "SELECT * FROM operador WHERE nomOp LIKE '%$busVervar%' OR apellidoPOp LIKE '%$busVervar%' OR apellidoMOp LIKE '%$busVervar%'"); //Busqueda por nombre y apellidos
             
            $contar = mysqli_num_rows($sql);
             
            if($contar == 0){
                  echo "No se han encontrado resultados para '<b>".$busVervar."</b>'.";
            }else{
